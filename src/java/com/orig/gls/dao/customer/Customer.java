...
public class Customer {
    ...
    public static String removeGroupMember(int groupId, String username) {
        String systemresponce = "failed";
        String s = "select no_of_members from groups_table where group_id=?";
        String str = AdminDb.getValue(s, 1, 1, String.valueOf(groupId));
        int k = 0;
        try {
        ...
        String sql = "update groups_table set no_of_members = ?, lchg_user_id =? where group_id=?";
        String in = k - 1 + "," + username + "," + groupId;
        if (AdminDb.dbWork(sql, 3, in) > 0) {
            systemresponce = "success";
        }

        return systemresponce;
    }

//    public static void main(String[] args) {
//        System.out.println(""+removeGroupMember(10, "MARK") );
//             
//    }
    ...
    public static int addAuditCustomerTrail(String acid) {
        ...
        for (int w = 0; w < ar.size(); w++) {
            ...
            String in = one.get(0) + "," + one.get(1) + "," + one.get(2) + ","
                    ...
                    + one.get(18) + "," + one.get(9) + "," + one.get(20)
                    + "," + one.get(21) + "," + one.get(22) + ","
                    + one.get(23) + "," + one.get(24)
                    + "," + one.get(25) + "," + one.get(26);
            String sql = "insert into GENERAL_ACCT_MAST_AUDIT_TABLE(ACID,ACCT_CRNCY_CODE,ACCT_MGR_USER_ID, ACCT_NAME,"
                    + "ACCT_OWNERSHIP,BANK_ID, CLR_BAL_AMT,CUST_ID, DEL_FLG,DRWNG_POWER,"
    ...
    public static ArrayList getAccountDetails(String foracid) {
        ...
        for (int h = 0; h < ar.size(); h++) {
            ...
            String subGroupName = getSubGroupName(subGroupCode);
            // System.out.println(accName+" "+" "+solId+" "+subGroupCode+" "+creditRating+" "+subGroupName);
            one.add(foracid);
            one.add(accName);
    ...
    public static void completeVerification(int foracid, String subGroupId, String actType, String uname) {
        ...
        String sql = "update general_acct_mast_table set member_status = ?, credit_rating = ?, sub_group_code = ?, lchg_user_id = ?, lchg_time = try_convert(date, getDate(), 111), last_modified_date = try_convert(date, getDate(), 111) where cust_id = ?";
