...
public class Mext {
    ...
    public static void exitMember(String custId, String uname) {
        ...
        String s = "insert into general_acct_mast_table_mod(ACID,ACCT_CRNCY_CODE,ACCT_MGR_USER_ID,ACCT_NAME,ACCT_OPN_DATE,ACCT_OWNERSHIP,BANK_ID,CLR_BAL_AMT,CUST_ID,DEL_FLG,DRWNG_POWER,ENTITY_CRE_FLG,FORACID,LAST_MODIFIED_DATE,LCHG_TIME,LCHG_USER_ID,LIEN_AMT,RCRE_TIME,RCRE_USER_ID,SANCT_LIM,SCHM_CODE,SCHM_TYPE,SOL_ID,GROUP_CODE,SUB_GROUP_CODE,LAST_OPER) values"
                + "(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        String in = "";
        for (int w = 0; w < 25; w++) {
            in = in + args[w] + ",";
