...
public class BeanFactory {
    ...
    public static Collection getRepaymentReport(String subGroupCode, Date fromDate, Date toDate) {
        String sql = "select tran_amt, tran_date, acid, rcre_user_id, lchg_user_id from history_transactions_table where sub_group_code = ? and tran_date between try_convert(date, ?, 111)  and try_convert(date, ?, 111) ";
    ...
    public static Collection getDisbursementReport(String subGroupCode, Date fromDate, Date toDate) {
        String sql = "select foracid, acct_name, dis_amt, dis_shdl_date from loan_disbursement_report where sub_group_code = ? and dis_shdl_date between try_convert(date, ?, 111)  and try_convert(date, ?, 111) ";
        String in = subGroupCode + "," + fromDate + "," + toDate;
