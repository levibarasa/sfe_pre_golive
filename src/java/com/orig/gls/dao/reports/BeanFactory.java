...
public class BeanFactory {
    ...
    public static Collection getCustomerReport(String subGroupCode, Date fromDate, Date toDate, String Status) {
        String sql = "select acct_name, clr_bal_amt, cust_id, foracid, schm_code, sol_id from general_acct_mast_audit_table where sub_group_code = ? "
                + "and last_modified_date between to_date(?, 'dd/MM/yyyy') and to_date(?,'dd/MM/yyyy') and member_status = ?";
    ...
    public static Collection getRepaymentReport(String subGroupCode, Date fromDate, Date toDate) {
        String sql = "select tran_amt, tran_date, acid, rcre_user_id, lchg_user_id from history_transactions_table where sub_group_code = ? and tran_date between to_date(?, 'dd/MM/yyyy') and to_date(?,'dd/MM/yyyy')";
    ...
    public static Collection getDisbursementReport(String subGroupCode, Date fromDate, Date toDate) {
        String sql = "select foracid, acct_name, dis_amt, dis_shdl_date from loan_disbursement_report where sub_group_code = ? and dis_shdl_date between to_date(?, 'dd/MM/yyyy') and to_date(?, 'dd/MM/yyyy')";
        String in = subGroupCode + "," + fromDate + "," + toDate;
    ...
    public static Collection getDemandsReport(String subGroupCode, Date fromDate, Date toDate) {
        String sql = "select foracid, acct_name, dmd_amt, dmd_date from loan_demands_report where sub_group_code = ? and dmd_date between to_date(?, 'dd/MM/yyyy') and to_date(?, 'dd/MM/yyyy')";
        String in = subGroupCode + "," + fromDate + "," + toDate;
        ArrayList col = AdminDb.execArrayLists(sql, 3, in, 4);
        ArrayList collection = new ArrayList();
        for (int w = 0; w < col.size(); w++) {
